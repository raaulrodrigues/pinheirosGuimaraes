<?php require_once 'header.php'; ?>
    <div class="dynamicSpacer"></div>
    <section id="default-hero">
        <div style="background-image: url('img/background-abous-us.jpg');">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="section-top">
                            <a href="#"><p>PRêMIOS E RECONHECIMENTOS</p></a>
                        </div>
                        <div class="default-tagline title-white">
                            <h1>Excelência  e Reconhecimento</h1>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section id="recognitions-info" class="bg-beige">
        <div class="spacer-60"></div>
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="recognitions-info-content">
                        <p>Ao longo dos anos, Pinheiro Guimarães tem sido considerado um dos líderes no Brasil em suas áreas de atuação, por publicações especializadas como Chambers & Partners, Euromoney/IFLR1000, Legal500, Who's Who Legal, LACCA, Análise Advocacia e Latin Lawyer.</p>
                    </div>
                </div>
            </div>
            <div class="spacer-60"></div>
        </div>
    </section>
    <section id="awards-grid">
        <div class="spacer-60"></div>
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="awards-grid-title title-green">
                        <h2>Reconhecimentos</h2>
                    </div>
                    <div class="awards-grid-years">
                        <span class="active">2024</span>
                        <span>2024</span>
                        <span>2024</span>
                        <span>2024</span>
                    </div>
                </div>
            </div>
            <div class="spacer-30"></div>
            <div class="row g-0">
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-chambers-brazil-2025.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-chambers-global-2025.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-legal-500.jpg" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-latin-lawyer-250-2026.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-iflr1000-2025.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-leading-firm-2026.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-brazil-legal-summit.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-legal-500.jpg" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-ttr.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-grr-100.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-lacca-2026.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-analise-advocacia-2026.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-refinitiv.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-lexology-index.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-latin-finance.png" alt="">
                        </a>
                    </div>
                </div>
                <div class="col-3">
                    <div class="award-grid-item">
                        <a href="#">
                            <img src="img/selo-itr-awards-2025.png" alt="">
                        </a>
                    </div>
                </div>
            </div>
            <div class="spacer-60"></div>
        </div>
    </section>
    <section id="awards-slider" class="bg-beige">
        <div class="spacer-60"></div>
        <div class="container">
            <div class="awards-slider-title title-green">
                <h2>Prêmios</h2>
            </div>
            <div class="awards-slider-years">
                <span class="active">2024</span>
                <span>2024</span>
                <span>2024</span>
                <span>2024</span>
            </div>
            <div class="spacer-30"></div>
            <div class="awards-slider-carousel">
                <div class="award-item">
                    <a href="#"><img src="img/selo-chambers-brazil-2025.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-chambers-global-2025.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-legal-500.jpg" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-latin-lawyer-250-2026.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-iflr1000-2025.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-leading-firm-2026.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-brazil-legal-summit.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-ttr.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-legal-500.jpg" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-grr-100.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-lacca-2026.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-analise-advocacia-2026.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-refinitiv.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-lexology-index.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-latin-finance.png" alt=""></a>
                </div>
                <div class="award-item">
                    <a href="#"><img src="img/selo-itr-awards-2025.png" alt=""></a>
                </div>
            </div>
            <div class="col-12 d-flex justify-content-start align-items-center mt-4">
                <div class="awards-prev slick-arrows">
                    <button type="button" class="slick-prev">Prev</button>
                </div>
                <div class="awards-next slick-arrows ms-3">
                    <button type="button" class="slick-next">Next</button>
                </div>
            </div>
            <div class="spacer-60"></div>
        </div>
    </section>
    <section id="useful-links" class="bg-beige">
        <div class="spacer-60"></div>
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="section-top-green text-green">
                        <span>LINKS ÚTEIS</span>
                    </div>
                </div>
            </div>
            <div class="spacer-60"></div>
            <div class="row">
                <div class="col-4">
                    <a href="#" class="link-card text-light-green text-hover-white bg-beige bg-hover-light-green">
                        <img src="img/icon-mail.svg" alt="">
                        <span>RECEBA NOSSO MAILING</span>
                    </a>
                </div>
                <div class="col-4">
                    <a href="#" class="link-card text-light-green text-hover-white bg-beige bg-hover-light-green">
                        <img src="img/icon-tel.svg" alt="">
                        <span>ENTRE EM CONTATO CONOSCO</span>
                    </a>
                </div>
                <div class="col-4">
                    <a href="#" class="link-card text-light-green text-hover-white bg-beige bg-hover-light-green">
                        <img src="img/icon-team.svg" alt="">
                        <span>VENHA FAZER PARTE DO NOSSO TIME</span>
                    </a>
                </div>
            </div>
            <div class="spacer-60"></div>
        </div>
    </section>
<?php require_once 'footer.php'; ?>